...
# // Create Instance Pool minmum 2 servers,maxmum 4, initial 2          
...
resource "oci_core_instance_pool" "CreateInstancePool" {
  ...
  placement_configurations {
    availability_domain = "${lookup(data.oci_identity_availability_domains.ADs.availability_domains[0],"name")}"
    primary_subnet_id   = "${oci_core_subnet.CreatePublicSubnet.id}"
  }
...
#      max    = "${var.autoscaling_policiesmax"
#      min    = "${var.autoscaling_policiesmin"
#     }
